...
size_t line_editor(uint8_t *buf, size_t buf_size, void *fileno, int (*escape_handler)(uint8_t code, line_editor_state_t *state))
{
...
int process_cli(uint8_t *buf, int size, cli_cmd_t *cli_cmd)
{
...
void process_command_prompt(cli_cmd_t *commands)
{
	...
	size = line_editor(buf, sizeof(buf), NULL, esc_handler);
