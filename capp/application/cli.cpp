...
#if 0
/*
	for each node we need a hash to associate chars with pointers
	what's a more compact encoding?
	list of key,value pairs where they are both bytes
	and a terminator avlue in the list

	preprocess with program
	insert in all keys and then print out tree

	let's assume max escape length is 8
	8 x 256 = 2K
...
size_t line_editor(char *buf, size_t buf_size, void *fileno, int (*escape_handler)(uint8_t code, line_editor_state_t *state))
{
	...
	while(1)
	{
		...
		/* Blocking read character from ring buffer */
